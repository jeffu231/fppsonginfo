...
If the change is part of a Github issue, add the appropriate Github reference. Fix #132

## Related To

A related to section should be added when the Github issue being worked on is related to another Github issue. The Related To
refers to the other issue(s). Do not add a related to section that relates to the same issue being committed.

## Conventional Commits

The start of the subject line must adhear to the the conventional commits guidelines. The commit hook will prevent invalid
commits if they are not structured correctly.
...
The Subject line must start with the conventional commit convention.
...
```
fix(Token): Prevent token expiration race condition
...
**Simple change**:
```
feat(Email): Add user email validation
...
**Bug fix**:
```
fix(Token): Fix race condition in token refresh
